...
BundleLocator.prototype = {
    ...
    _walkNPMTree: function (dir, _depth) {
        ...
        if (_depth < this._options.maxPackageDepth) {
            try {
                subdirs = libfs.readdirSync(libpath.join(dir, 'node_modules', scope));
            } catch (readdirErr) {
            ...
            if (!scope) {
                subdirs = subdirs.reduce(function (memo, subdir) {
                    return memo.concat(
                        '@' === subdir.substring(0, 1) ?
                            libfs.readdirSync(libpath.join(dir, 'node_modules', subdir)).map(function (scopeddir) {
                                return libpath.join(subdir, scopeddir);
                            }) :
                            [subdir]
                    );
                }, [])
            ...
            subdirs.forEach(function (subdir) {
                ...
                subpkgResults = self._walkNPMTree(libpath.join(dir, 'node_modules', scope, subdir), _depth + 1);
